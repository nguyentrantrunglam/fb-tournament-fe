...
## Migration progress (full-rewire mode, 2026-06-03)

User chose **full rewire off Firebase**. Safe execution order = build all NestJS API modules (server-side, no web breakage) → ONE coordinated web cutover → remove Firebase. Web currently still 100% Firebase + working (no regression).

**Done + committed (`badminton-api`):**
- ✅ P01 foundation (session/passport/socket.io/mongo/guards/docker) — `d94f71b`
- ✅ P02 API auth + users/admin (register/login/me/logout/forgot/reset, PII isolation) — `bf03182`
- ✅ P03 API tournaments + categories + courts + storage-presign + real TournamentRoleGuard — `bf03182`
- ✅ Code-review + Critical/High security fixes (presign IDOR, content-type, CastError→400, authz) — `8e4578a`
- e2e **34/34** green; build/unit green.
