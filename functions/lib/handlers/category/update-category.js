...
exports.updateCategory = (0, https_1.onCall)({ region: admin_1.REGION }, async (req) => {
    const d = (req.data ?? {});
    const tid = String(d.tournamentId ?? '');
    const cid = String(d.categoryId ?? '');
    await (0, role_guard_1.assertTournamentAccess)(req, tid);
    if (!cid)
        throw new https_1.HttpsError('invalid-argument', 'Thiếu categoryId');
    ...
    const ref = admin_1.db.doc(`tournaments/${tid}/categories/${cid}`);
    const snap = await ref.get();
    if (!snap.exists)
        throw new https_1.HttpsError('not-found', 'Hạng mục không tồn tại');
    await ref.set({ code, name, playerCount, genderRequirement: gender, bestOf, fee, maxTeams, registrationDeadline: deadline }, { merge: true });
    return { ok: true };
});
