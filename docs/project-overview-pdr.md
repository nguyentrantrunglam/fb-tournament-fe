...
> **Status:** Draft v0.1 — Locked sau khi user xác nhận
> **Ngày:** 2026-05-28
...
| Migrate backend riêng (Node.js + WS) không phải viết lại business logic | Bracket / scoring logic ở Cloud Functions portable |

## 3. Người dùng & vai trò

### Global roles (gắn với user)
| Role | Quyền |
|---|---|
| `admin` | Quản trị platform: cấp quyền `organizer`, ban user, xem mọi giải |
| `user` (default) | Mọi tài khoản mặc định. Có thể đăng ký vào giải. |
...
### Auth & User
- Đăng ký bằng email + password (Firebase Auth).
- Đăng nhập Google (Firebase Auth provider).
- Profile bắt buộc lúc đăng ký:
  - Họ tên
  - **CCCD (Căn Cước Công Dân) 12 số** — bắt buộc, unique toàn hệ thống
  - Giới tính (nam / nữ)
  - Ngày sinh
- Profile tùy chọn: số điện thoại, avatar.
- **Mỗi người chỉ có 1 tài khoản duy nhất**, định danh qua CCCD. Hệ thống reject signup nếu CCCD đã tồn tại.
- CCCD chỉ chính chủ + admin xem được; KHÔNG hiển thị public, không hiển thị cho organizer / referee / đồng đội.
...
**Combination invalid:** `genderRequirement = mixed_pair` AND `playerCount = 1` → CF reject lúc tạo category.
...
- **Số lượng đội tối đa** (`maxTeams`): hard cap.
  ...
  - Đầy slot → CF reject đăng ký mới.
...
- Khi BTC submit bulk → CF xử lý từng dòng:
...
**Quy tắc edit điểm:**
- Trọng tài: chỉ sửa điểm match mình ghi (`uid == match.refereeUid`), **trong 24h sau `endedAt`**.
...
| **P5** | Mời trọng tài qua link / email. Re-arrange bracket UI tốt hơn. Conflict detection lịch nâng cao. |
| **P6** | ELO/ranking nội bộ. Lịch sử thành tích VĐV. Đề xuất seed tự động. |
| **P7** | Migrate BE: Node.js + WebSocket cho realtime score. Bracket service tách riêng. |
| **P8** | Payment đăng ký. Hoá đơn / refund. |
...
| **P3** | Public view + Firestore realtime + Demo chạy 1 giải thật | 1 tuần |
...
| # | Quyết định | Lý do |
|---|---|---|
| D1 | Stack: Next.js + Firebase | Tốc độ build MVP, realtime built-in, free tier dư dả |
| D2 | Cloud Functions viết portable (logic thuần) | Migrate Node.js + WS sau không phải viết lại |
...
| D11 | 1 collection `users` duy nhất, role scoped | KISS, mọi user đều là VĐV |
...
| D30 | Hạn đăng ký là gate cứng (CF reject sau deadline), không auto đóng | KISS, BTC quyết định lúc nào đóng. |
...
|---|---|---|---|
| Firestore transaction giới hạn 500 docs/lần khi gen bracket lớn | Trung bình | Cao | Batch theo round; bracket > 64 người cảnh báo BTC; chuyển sang Cloud Functions logic |
| Cost Firestore tăng nhanh nếu viral | Thấp | Trung bình | Theo dõi billing alert; denormalize cẩn thận; cache RSC trên Vercel |
| Re-arrange bracket gây data inconsistency | Trung bình | Cao | Versioning + atomic transaction; chỉ cho re-arrange trước match đầu tiên `in_progress` |
...
| Vendor lock-in Firebase | Cao | Trung bình | Cloud Functions logic portable (D2); domain layer không bind SDK |
...
6. 10 user đăng ký `MS` (1 user trùng CCCD → reject). 4 cặp đăng ký `MX` (1 cặp mixed sai gender → CF reject). 5 cặp đăng ký `OPEN` (không validate gender, đều OK gồm 2 cặp nam-nam, 2 cặp nữ-nữ, 1 cặp mixed).
...
7.1. User A dùng **bulk register** tạo 6 dòng (5 cho `MS`, 1 cho `MX`): trong đó 1 dòng `MX` chọn 2 user nam → CF reject riêng dòng đó; 5 dòng `MS` valid → auto `approved`. UI hiển thị bảng: 5 success + 1 error rõ ràng.
8. User A thử đóng `MS` khi còn 2 pending → CF reject "Còn 2 đăng ký pending". User A duyệt nốt → đóng `MS` thành công.
...
**Next artifact:** [docs/system-architecture.md](system-architecture.md) — ERD, Firestore schema, Cloud Functions structure, security rules.
